...
### 16-c. 데이터 처리 코드를 캡슐화 하기 : 배열 대신 연결리스트 사용하기
...
### 23. 예외가 발생했을 때 시스템을 멈추지 않게 하는 방법

### 24-a. 'Command' 디자인 패턴을 적용하기 : 적용 전 문제점 확인
...
### 29-b. 파일 관리를 별도의 애플리케이션으로 분리하기 : 간단한 메시지 송수신
### 29-c. 파일 관리를 별도의 애플리케이션으로 분리하기 : 사용자가 입력한 명령처리
...
### 34-a. 객체 생성을 자동화하기 : 프록시 패턴을 이용한 DAO 구현체 자동 생성
### 34-b. 객체 생성을 자동화하기 : .propertis 파일을 이용한 Command 구현체 자동 생성
### 34-c. 객체 생성을 자동화하기 : 애노테이션을 이용한 Command 구현체 자동 생성
...
### 35-d. 애플리케이션을 클라이언트/서버 구조로 변경하기 : 멀티스레드 환경에서 DB 커넥션을 다루는 방법
### 35-e. 애플리케이션을 클라이언트/서버 구조로 변경하기 : 
### 35-f. 애플리케이션을 클라이언트/서버 구조로 변경하기 : 
